...
export class FilterAdvanceComponent {
  ...
  getNewTotal(PRODUCT: any, DISCOUNT_FLASH_P: any) {
    if (this.currency == 'PEN') {
      return DISCOUNT_FLASH_P.type_discount == 1
        ? (PRODUCT.price_pen - PRODUCT.price_pen * (DISCOUNT_FLASH_P.discount * 0.01)).toFixed(2)
        : (PRODUCT.price_pen - DISCOUNT_FLASH_P.discount).toFixed(2);
    } else {
      return DISCOUNT_FLASH_P.type_discount == 1
        ? (PRODUCT.price_usd - PRODUCT.price_usd * (DISCOUNT_FLASH_P.discount * 0.01)).toFixed(2)
        : (PRODUCT.price_usd - DISCOUNT_FLASH_P.discount).toFixed(2);
    }
  }
